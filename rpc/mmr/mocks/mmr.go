...
// GenerateBatchProof provides a mock function with given fields: indices, blockHash
func (_m *MMR) GenerateBatchProof(indices []uint64, blockHash types.Hash) (types.GenerateMmrBatchProofResponse, error) {
	ret := _m.Called(indices, blockHash)

	var r0 types.GenerateMmrBatchProofResponse
	if rf, ok := ret.Get(0).(func([]uint64, types.Hash) types.GenerateMmrBatchProofResponse); ok {
		r0 = rf(indices, blockHash)
	} else {
		r0 = ret.Get(0).(types.GenerateMmrBatchProofResponse)
	}

	var r1 error
	if rf, ok := ret.Get(1).(func([]uint64, types.Hash) error); ok {
		r1 = rf(indices, blockHash)
	} else {
		r1 = ret.Error(1)
	}
